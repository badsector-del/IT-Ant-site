...
const { data: membership } = await db.from('company_users').select('companies(name,pib,address,bank_name,bank_account,tax_regime,vat_number)').eq('company_id', window.itAntActiveCompanyId).single();
