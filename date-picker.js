...
function initializeDatePickers() {
  document.querySelectorAll('[data-date-picker]').forEach(picker => {
    if (picker.dataset.datePickerReady === 'true') return;
    ...
    display.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); calendar.hidden = !calendar.hidden; render(); } });
    calendar.addEventListener('click', event => {
      event.stopPropagation();
      const day = event.target.closest('[data-date]');
      if (day) { setValue(day.dataset.date); return; }
      if (event.target.closest('[data-calendar-prev]')) { viewDate.setMonth(viewDate.getMonth() - 1); render(); }
      if (event.target.closest('[data-calendar-next]')) { viewDate.setMonth(viewDate.getMonth() + 1); render(); }
    });
...
window.setDatePickerValue = (name, value) => {
  initializeDatePickers();
  const picker = document.querySelector(`[data-date-picker="${name}"]`);
  if (!picker) return;
  const display = picker.querySelector('[data-date-display]');
  const hidden = picker.querySelector('[data-date-value]');
  if (!display || !hidden) return;
  ...
  const calendar = picker.querySelector('.date-calendar');
