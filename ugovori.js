...
const billingTypeSelect = form.elements.namedItem('billing_type');
const subtotalLabel = subtotalInput.closest('label');
const totalLabel = totalInput.closest('label');
const planFields = document.createElement('div');
planFields.className = 'contract-plan-fields';
planFields.innerHTML = '<p class="tax-note">Plan naplate po mesecima</p><div class="contract-plan-list" id="contract-plan-list"></div>';
vatLabel.before(planFields);
const planList = planFields.querySelector('#contract-plan-list');
...
function renderPlanRows(values = {}) { const year = new Date().getFullYear(); planList.innerHTML = monthNames.map((name, index) => `<label><span>${name} ${year}</span><input type="number" min="0" step="0.01" data-plan-month="${index + 1}" value="${values[index + 1] ?? ''}" placeholder="0,00"></label>`).join(''); }
function readPlanRows() { const rate = taxRegime === 'books_vat' ? Number(vatRateSelect.value || 0) : 0; return [...planList.querySelectorAll('[data-plan-month]')].map(input => { const subtotal = Number(input.value || 0); const vatAmount = subtotal * rate / 100; return { billing_year: new Date().getFullYear(), billing_month: Number(input.dataset.planMonth), subtotal, vat_rate: rate, vat_amount: vatAmount, total: subtotal + vatAmount }; }); }
function toggleBillingType() { const variable = billingTypeSelect.value === 'variable'; planFields.hidden = !variable; subtotalLabel.hidden = variable; totalLabel.firstChild.textContent = variable ? 'Prosečno ukupno mesečno' : 'Ukupno mesečno'; if (variable && !planList.children.length) renderPlanRows(); if (!variable) calculateTotal(); }
...
async function openModal(contract = null) { modal.hidden = false; editingId = contract?.id || null; form.reset(); durationSelect.value = contract?.duration_type || 'indefinite'; endDateLabel.hidden = durationSelect.value !== 'definite'; if (typeof window.setDatePickerValue === 'function') { window.setDatePickerValue('contract_signature_date', contract?.signature_date || today()); window.setDatePickerValue('contract_end_date', contract?.end_date || ''); } form.elements.namedItem('number').value = contract?.number || ''; form.elements.namedItem('status').value = contract?.status || 'active'; billingTypeSelect.value = contract?.billing_type || 'fixed'; subtotalInput.value = contract?.monthly_subtotal ?? ''; vatRateSelect.value = String(contract?.vat_rate ?? (taxRegime === 'books_vat' ? 20 : 0)); form.elements.namedItem('item_prefix').value = contract?.item_prefix || ''; form.elements.namedItem('notes').value = contract?.notes || ''; document.querySelector('#contract-title').textContent = editingId ? 'Izmeni ugovor' : 'Novi ugovor'; document.querySelector('.modal-submit').textContent = editingId ? 'Sačuvaj izmene' : 'Sačuvaj ugovor'; await loadClients(contract?.client_id || ''); if (contract?.id) { const { data: plan } = await db.from('contract_billing_plan').select('billing_month,subtotal').eq('contract_id', contract.id).eq('billing_year', new Date().getFullYear()); renderPlanRows(Object.fromEntries((plan || []).map(item => [item.billing_month, item.subtotal]))); } else renderPlanRows(); toggleBillingType(); calculateTotal(); clientSelect.focus(); }
form.addEventListener('submit', async event => { event.preventDefault(); try { if (!companyId) return alert('Aktivno preduzeće nije učitano.'); if (!clientSelect.value) return alert('Izaberite komitenta.'); const signatureDate = form.signature_date.value; if (!signatureDate) return alert('Izaberite datum potpisa.'); if (durationSelect.value === 'definite' && !form.end_date.value) return alert('Izaberite datum završetka.'); const variable = billingTypeSelect.value === 'variable'; const plan = variable ? readPlanRows() : []; const averageSubtotal = variable ? plan.reduce((sum, item) => sum + item.subtotal, 0) / plan.length : Number(subtotalInput.value || 0); const rate = taxRegime === 'books_vat' ? Number(vatRateSelect.value || 0) : 0; if (averageSubtotal < 0 || plan.some(item => item.subtotal < 0)) return alert('Iznos ne može biti negativan.'); const vatAmount = averageSubtotal * rate / 100; const payload = { company_id: companyId, client_id: clientSelect.value, number: form.number.value.trim() || null, signature_date: signatureDate, duration_type: durationSelect.value, end_date: durationSelect.value === 'definite' ? form.end_date.value : null, status: form.status.value, billing_type: billingTypeSelect.value, monthly_subtotal: averageSubtotal, vat_rate: rate, vat_amount: vatAmount, monthly_total: averageSubtotal + vatAmount, tax_regime: taxRegime, item_prefix: form.item_prefix.value.trim() || null, notes: form.notes.value.trim() || null, updated_at: new Date().toISOString() }; if (!editingId && !payload.number) { const { data: number, error: numberError } = await db.rpc('next_contract_number'); if (numberError) throw numberError; payload.number = number; } const result = editingId ? await db.from('contracts').update(payload).eq('id', editingId).select('id').single() : await db.from('contracts').insert(payload).select('id').single(); if (result.error) throw result.error; const contractId = editingId || result.data.id; const { error: planDeleteError } = await db.from('contract_billing_plan').delete().eq('contract_id', contractId); if (planDeleteError) throw planDeleteError; if (variable) { const { error: planError } = await db.from('contract_billing_plan').insert(plan.map(item => ({ ...item, contract_id: contractId }))); if (planError) throw planError; } modal.hidden = true; await loadContracts(); } catch (error) { alert(`Ugovor nije sačuvan: ${error?.message || error}`); } });
durationSelect.addEventListener('change', () => { endDateLabel.hidden = durationSelect.value !== 'definite'; }); billingTypeSelect.addEventListener('change', toggleBillingType);
