  ...
  try {
    ...
    if (body.action === 'list') {
      const [{ data: companies, error: companiesError }, { data: memberships, error: membersError }, { data: users, error: usersError }, { data: logs, error: logsError }] = await Promise.all([
        admin.from('companies').select('id,name,pib,mb,activity_code,responsible_person,address,bank_name,bank_account,tax_regime,vat_number,regime_effective_from,created_at').order('created_at'),
    ...
    if (body.action === 'create-company') {
      ...
      const { data, error } = await admin.from('companies').insert({ name: body.name.trim(), pib: body.pib?.trim() || null, mb: body.mb?.trim() || null, activity_code: body.activity_code?.trim() || null, responsible_person: body.responsible_person?.trim() || null, address: body.address?.trim() || null, bank_name: body.bank_name?.trim() || null, bank_account: body.bank_account?.trim() || null, tax_regime: body.tax_regime || 'pausal', vat_number: body.vat_number?.trim() || null, regime_effective_from: body.regime_effective_from || new Date().toISOString().slice(0, 10) }).select('id').single();
    ...
    if (body.action === 'update-company') {
      ...
      const { data, error } = await admin.from('companies').update({ name: body.name.trim(), pib: body.pib?.trim() || null, mb: body.mb?.trim() || null, activity_code: body.activity_code?.trim() || null, responsible_person: body.responsible_person?.trim() || null, address: body.address?.trim() || null, bank_name: body.bank_name?.trim() || null, bank_account: body.bank_account?.trim() || null, tax_regime: body.tax_regime || 'pausal', vat_number: body.vat_number?.trim() || null, regime_effective_from: body.regime_effective_from || new Date().toISOString().slice(0, 10) }).eq('id', body.company_id).select('id').single();
      if (error) throw error;
