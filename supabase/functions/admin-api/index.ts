...
Deno.serve(async request => {
  ...
  try {
    ...
    if (body.action === 'list') {
      const [{ data: companies, error: companiesError }, { data: memberships, error: membersError }, { data: users, error: usersError }] = await Promise.all([
      ...
      if (companiesError || membersError || usersError) throw new Error('Učitavanje administracije nije uspelo.');
      const emails = new Map(users.users.map(item => [item.id, item.email || item.id]));
      return json({ companies, memberships: memberships.map(item => ({ ...item, email: emails.get(item.user_id) })) });
    }
